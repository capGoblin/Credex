#!/usr/bin/env node
/**
 * Credex Client Agent Server
 *
 * HTTP server for the Credex Client Agent.
 * Run with: npm start
 */
...
import { credexClientAgent } from "./agent";
import "dotenv/config";

const PORT = parseInt(process.env.PORT || "10004");
...
const runner = new Runner({
  appName: "credex_client",
  agent: credexClientAgent,
  sessionService,
  artifactService,
  memoryService,
});

console.log("🚀 Starting Credex Client Agent Server...");
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(JSON.stringify({ status: "ok", service: "credex-client-agent" }));
    return;
  }
...
server.listen(PORT, () => {
  console.log(`✅ Credex Client Agent running at http://localhost:${PORT}`);
  console.log(
    `\nTest with: curl -X POST http://localhost:${PORT} -H "Content-Type: application/json" -d '{"text": "check my limit"}'`,
  );
});
